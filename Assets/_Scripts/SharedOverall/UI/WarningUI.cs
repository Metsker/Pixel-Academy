...
namespace _Scripts.SharedOverall.UI
{
    public class WarningUI : UIPanel
    {
        ...
        [SerializeField] private BaseRewardedAdVideo baseRewardedAdVideo;
        ...
        public void OnButtonClick()
        {
            switch (_warningType)
            {
                ...
                case WarningType.Skip:
                    ...
                    break;
                case WarningType.OpaqueHint:
                    switch (baseRewardedAdVideo.IsAdLoaded())
                    {
                        case true:
                            _opaqueHint.GetHintByAd();
                            break;
                        case false:
                            baseRewardedAdVideo.RequestRewarded();
                            return;
                    }
                    if (!showAgain.isOn) break;
                    PlayerPrefs.SetInt("OpaqueHintWarning", 1);
                    PlayerPrefs.Save();
                    break;
                case WarningType.ClipHint:
                    switch (baseRewardedAdVideo.IsAdLoaded())
                    ...
                    if (!showAgain.isOn) break;
                    PlayerPrefs.SetInt("ClipHintWarning", 1);
                    PlayerPrefs.Save();
                    break;
                case WarningType.Exit:
